...
p = Path('server.js')
s = p.read_text()
if '// LOGIN_BOARD_PERSISTENCE_V1' in s:
...
'''
    rep('db insertion', "const players = new Map();\nconst slimes = new Map();", db_code + "\nconst players = new Map();\nconst slimes = new Map();")
    rep('player identity', "id, map: 'forest', x: p.x, y: p.y,", "id, nickname: 'Guest-' + id.slice(0, 4).toUpperCase(), userId: null, persistedPlaySeconds: 0, playSessionStartedAt: Date.now(), boardSeenId: latestBoardPostId, lastBoardPostAt: 0, map: 'forest', x: p.x, y: p.y,")
    rep('serialize nickname', "return { id: p.id, map: p.map, x: p.x, y: p.y, direction: p.direction, moving: p.moving, avatar: p.avatar, hp: p.hp, maxHp: p.maxHp, alive: p.alive, ghostShip: p.ghostShipUntil > Date.now(), ghostDirX: p.ghostDirX, ghostDirY: p.ghostDirY };", "return { id: p.id, nickname: p.nickname, map: p.map, x: p.x, y: p.y, direction: p.direction, moving: p.moving, avatar: p.avatar, hp: p.hp, maxHp: p.maxHp, alive: p.alive, ghostShip: p.ghostShipUntil > Date.now(), ghostDirX: p.ghostDirX, ghostDirY: p.ghostDirY };")
    rep('state profile', "bossProgress, bossTarget: BOSS_TARGET, bossActive: bossId !== null\n  };", "bossProgress, bossTarget: BOSS_TARGET, bossActive: bossId !== null,\n    profile: { nickname: viewer.nickname, loggedIn: !!viewer.userId, playSeconds: currentPlaySeconds(viewer) },\n    board: viewer.map === BOARD.map ? { x: BOARD.x, y: BOARD.y, hasNew: latestBoardPostId > Number(viewer.boardSeenId || 0) } : null\n  };")
    rep('welcome profile', "id: p.id, map: p.map, mapName: MAPS.forest.name,", "id: p.id, nickname: p.nickname, loggedIn: false, playSeconds: 0, map: p.map, mapName: MAPS.forest.name,")

    handlers = r'''  io.emit('count', { current: players.size, max: MAX_PLAYERS });
...
'''
    rep('ui handlers', "addEventListener('keydown',e=>{const k=e.key.toLowerCase();", ui + "addEventListener('keydown',e=>{const k=e.key.toLowerCase();")
    rep('ui keyguard', "addEventListener('keydown',e=>{const k=e.key.toLowerCase();if(['w','a','s','d'", "addEventListener('keydown',e=>{const k=e.key.toLowerCase();if(k==='escape'){authPanel.style.display='none';boardPanel.style.display='none';return;}if(uiOpen())return;if(['w','a','s','d'")
    rep('board F', "if(k==='f'&&!e.repeat){tryPortal();e.preventDefault();}", "if(k==='f'&&!e.repeat){if(nearBoard())socket.emit('openBoard');else tryPortal();e.preventDefault();}")
    rep('pause input UI', "setInterval(()=>{if(serverFull)return;let x=0,y=0;", "setInterval(()=>{if(serverFull)return;let x=0,y=0;if(uiOpen()){if(lastInputX!==0||lastInputY!==0){socket.emit('input',{x:0,y:0});lastInputX=0;lastInputY=0;}return;}")
    rep('init persistence', "server.listen(PORT, () => {\n  console.log('Forest RPG running on port ' + PORT);", "initPersistence().finally(() => server.listen(PORT, () => {\n  console.log('Forest RPG running on port ' + PORT);")
    rep('listen close', "console.log('Slimes ' + NORMAL_SLIMES + ' normal + ' + ELITE_SLIMES + ' elite');\n});", "console.log('Slimes ' + NORMAL_SLIMES + ' normal + ' + ELITE_SLIMES + ' elite');\n  console.log('Accounts/board persistence: ' + (dbReady ? 'PostgreSQL' : 'memory fallback'));\n}));")
    p.write_text(s)

pkgp = Path('package.json')
pkg = json.loads(pkgp.read_text())
pkg.setdefault('dependencies', {})['pg'] = '^8.13.1'
